...
// Problem URL: https://www.codewars.com/kata/537e18b6147aa838f600001b/javascript
...
function justify(text, width) {
  ...
  var lastLine = words.reduce(function (line, word) {
    if (line) {
      if (line.length + word.length + 1 <= width) return line + " " + word;
      lines.push(line);
    }
    return word;
  });
  lines = lines.map(function (line) {
    if (line.indexOf(" ") >= 0)
      for (var lineLen = line.length; lineLen < width; )
        line = line.replace(/ +/g, function (spaces) {
          return spaces + (lineLen++ < width ? " " : "");
        });
    return line;
  });
