...
// Problem URL: https://www.codewars.com/kata/671bd5419ea261fbb8d0a0ca/javascript

function decipher(code) {
    let alphabets = 'abcdefghijklmnopqrstuvwxyz'.split('');
    let accum = 0, avg = 0, cipher = '';
    for (let i = 0; i < code[0].length; i++) {
        for (let j = 0; j < code.length; j++) {
            accum += (alphabets.indexOf(code[j][i]) + 1) || 0;
        }
        avg = accum / code.length;
        cipher += alphabets[Math.floor(avg) - 1] || ' ';
        accum = 0;
        avg = 0;
    }
    return cipher;
